...
int calc_store_persistent_string(CalcSession *session, const char *var_name, const char *payload)
{
    ...
    int status = build_string_entry(session, var_name, payload, &content);
    if (status == APP_OK)
    {
...
{
    FileContent *content = NULL;
    int status = build_binary_entry(session, var_name, payload, payload_len, &content);
    if (status == APP_OK)
    {
        int transfer_result = ticalcs_calc_send_var(session->calc, MODE_NORMAL, content);
        if (transfer_result != 0)
...
static int build_string_entry(CalcSession *session, const char *var_name, const char *payload, FileContent **out_content)
{
    ...
    *out_content = NULL;
    ...
    if ((strncmp(var_name, "Str", 3) != 0) || (strlen(var_name) != 4) || (!isdigit((unsigned char)var_name[3])))
    {
        fprintf(stderr, "Unsupported string variable name '%s'\n", (var_name != NULL) ? var_name : "(null)");
        return APP_ERR_IO;
    }

    if ((var_name[3] < '0') || (var_name[3] > '9'))
    {
        fprintf(stderr, "String index out of range in '%s'\n", var_name);
        return APP_ERR_IO;
    }
    ...
    if (ticonv_charset_utf16_to_ti_s(session->calc_model, utf16_payload, ti_payload) == NULL)
    {
        status = APP_ERR_IO;
        goto cleanup;
    }
